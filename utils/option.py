...
def get_args_parser():
    ...
    parser.add_argument('--img-size', default=[512, 64], type=int, nargs='+', help='image size [W, H], default: 512x64')
    parser.add_argument('--attn-mask-ratio', default=0., type=float, help='attention drop_key mask ratio')
    # 注意: patch-size 参数目前未在 create_model 中使用，模型内部硬编码为 (4, 64)
    parser.add_argument('--patch-size', default=[4, 64], type=int, nargs='+', help='patch size (currently not used)')
    parser.add_argument('--mask-ratio', default=0.4, type=float, help='mask ratio (论文规范: 0.4)')
    parser.add_argument('--cos-temp', default=8, type=int, help='cosine similarity classifier temperature')
    parser.add_argument('--max-span-length', default=8, type=int, help='span length for masking (论文规范: 8)')
